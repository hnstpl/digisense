...
{
    public class OldTractorEvaluationController : Controller
    {

        private readonly IOldTractorEvaluation _OldTractorEvaluation;
        ...
        public OldTractorEvaluationController(IOldTractorEvaluation OldTractorEvaluation, IGlobalService Global)
        {
            _OldTractorEvaluation = OldTractorEvaluation;
            _global = Global;
        }
        ...
        [HttpGet]
        public ActionResult OldTractorEvaluation()
        {
          
            ViewBag.LangListItem = _OldTractorEvaluation.GetLanguagedata().ToList();
            ViewBag.ImpModelData = _OldTractorEvaluation.GetModeldata();
            int langid = 1;
            ...
            try
            {
                ...
                ViewBag.Enquirylist = _OldTractorEvaluation.GetOldTractordata(langid);
                ...
                HttpContext.Session.SetString("modeltype", "");
                return View();
        ...
        [HttpPost]
        public ActionResult OldTractorEvaluation(IFormCollection formCollection)
        {
            ...
            {
                // Enquiry = Enquiry.Where(x => x.ProductNImplement == Convert.ToInt32(formCollection["cmbprodtype"])).ToList();
            ...
            try
            {
                ...
                ViewBag.Enquirylist = _OldTractorEvaluation.GetOldTractordatabySearch(langid, fromDate, toDate, enqtype, prodtype);
        ...
        public ContentResult Updateenquiry(IFormCollection formCollection)
        {
            ...
            try
            {
                ...
                var UpdateStatus = _OldTractorEvaluation.Updateenquiry(enquiryid, active, updateduser);
        ...
        public MemoryStream DownloadTemplate(bool isEmpty)
        {
            ...
            {
                xlPack.Workbook.Properties.Title = "Admin  - OldTractor Evaluation / Interested in Exchange Enquiry";
                ...
                //Set Title 
                var cell = xlSheet.Cells[1, 1, 1, 14];
                xlSheet.Row(1).Height = 28;
                cell.Value = "Admin  - OldTractor Evaluation / Interested in Exchange Enquiry";
                cell.Merge = true;
                ...
                HeaderRow(xlSheet, 3, 07, 4, 07, "Interested In", true);
                HeaderRow(xlSheet, 3, 08, 4, 08, "Interested Model", true);
                HeaderRow(xlSheet, 3, 09, 4, 09, "Enquired By", true);
                HeaderRow(xlSheet, 3, 10, 4, 10, "Enquired Date", true);
                HeaderRow(xlSheet, 3, 11, 4, 11, "RC Available", true);
                HeaderRow(xlSheet, 3, 12, 4, 12, "Year Of Manufacture", true);
                HeaderRow(xlSheet, 3, 13, 4, 13, "Image", true);
                HeaderRow(xlSheet, 3, 14, 4, 14, "Remarks", true);
                ...
                if (isEmpty)
                {
                    ...
                    int dno = 0;
                    foreach (OldTractorEvaluation od in value)
                    {
                        dno++;
                        ...
                        ColIndex++;
                        CreateExcelData(xlSheet, Rowindex, ColIndex, Convert.ToString(od.Intrested_IN == 1 ? "Tractors" : "Implements"), false);
                        ColIndex++;
                        CreateExcelData(xlSheet, Rowindex, ColIndex, Convert.ToString(od.Intrested_IN_TPDH_MODELNAME_VC), false);
                        ColIndex++;
                        CreateExcelData(xlSheet, Rowindex, ColIndex, Convert.ToString(od.CustName_VC), false);
                        ColIndex++;
                        CreateExcelData(xlSheet, Rowindex, ColIndex, Convert.ToDateTime(od.CREATEDDT_D).ToString("dd-MMM-yyyy"), false);
                        ColIndex++;
                        CreateExcelData(xlSheet, Rowindex, ColIndex, Convert.ToString(od.ISRCAvailable == "Y" ? "YES" : "NO"), false);
                        ColIndex++;
                        CreateExcelData(xlSheet, Rowindex, ColIndex, Convert.ToString(od.Year_OF_MANUFACTURE), false);
                        ColIndex++;
                        if (od.ImageData.Count > 0)
                        {
                            foreach (var itemimage in od.ImageData)
                            {
                                if (itemimage.Imagepath != "" && itemimage.Imagepath != null)
                                {
                                    CreateExcelData(xlSheet, Rowindex, ColIndex, Convert.ToString(itemimage.Imagepath), false);
                                   
                                }
                                else
                                {
                                    CreateExcelData(xlSheet, Rowindex, ColIndex, "", false);
        ...
        public List<OldTractorEvaluation> GetEnquirydata()
        {
            ...
            try
            {
                ...
                result = _OldTractorEvaluation.GetOldTractordatabySearch(langid, fromDate, toDate, Enqtype, prodtype).ToList();
                return result;
            }
            catch (Exception ex)
            {
